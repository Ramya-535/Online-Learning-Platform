...
const {register:registerSubject, handleSubmit:handleSubmitSubject}=useForm();
...
try {
  ...
  return (
    <div>
                ...
                <div className="col-12">
                  ...
                  //  {...registerSubject("facultyId")} 
                   className=" form-select "  onChange={(e) => {
                    const [facultyId, facultyName1] = e.target.value.split('_');
                    console.log(facultyId, "data");
                    setFacultyName(facultyName1);
                    setFacultyId(facultyId)
                  }}>
                    <option value="" >---Select---</option>
                    {faculties&&faculties.map((item)=><option key={item.id} value={`${item.facultyId}_${item.facultyName}`} >{item.facultyName}</option>)}
                  </select>
                </div>
                <div className="col-12">
      ...
      <select name="" className='form-control' required onChange={(e)=>setSelectSemester(e.target.value)}>
        <option value="" >Select </option>
        ...
        <option value="even">Even</option>
                ...
                <div className="col-12">
                 {selectSemester==="odd" &&<div className="col-lg-6 mt-1 ">
<label className='form-label'>
Odd Semester
</label> 
<select  className='form-control'  name="" required onChange={(e)=>setYears(e.target.value)}  >
  <option value="">Select </option>
  ...
  <option value="IV-I">IV-I</option>
    ...
    {selectSemester==="even" &&<div className="col-lg-6 mt-1 ">
      <label className='form-label'>
   Even Semester
      </label>
      <select  className='form-control' name=""  required  onChange={(e)=>setYears(e.target.value)}>
       ...
       {years&&        <div className="col-12">
                  <label htmlFor="" className="form-label">
                    Subject
                  </label>
 <select name="" className='form-select'  {...registerSubject("subjects")}>
 <option value="">----Select ----</option>
 {subjects1.map((item)=>(item.year===years&&<option value={item.name} key={item.name}>
{item.name}
  </option>))}
</select>
                  {/* <Select
